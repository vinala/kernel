...
class Libs
{
	public static function css($file,$default=true)
	{
		...
		else
		{
			...
			else echo '<link rel="stylesheet" type="text/css" href="'.$file.'">'."\n";
		}
	}

	public static function js($file,$default=true)
	{
		if (strpos($file,'http://') !== false) echo '<script type="text/javascript"  src="'.$file.'"></script>'."\n";
		else
		{
			if($default)
			{
			...
			else echo '<script type="text/javascript"  src="'.$file.'"></script>'."\n";
		}
