...
class Router
{
    ...
    public static function get($uri, $callback, $subdomains = null)
    {
        ...
        if (!is_null($subdomains)) {
            $domains = self::extractSubdomains($subdomains);
        }
        if (is_callable($callback)) {
            self::addCallable($uri, $callback, 'get', $domains);
        } elseif (is_array($callback)) {
            self::addFiltred($uri, $callback, 'get', $domains);
        }
    }
    ...
    {
        if (is_callable($callback)) {
            self::addCallable($uri, $callback, 'post');
        } elseif (is_array($callback)) {
            self::addFiltred($uri, $callback, 'post');
        }
    }
    ...
    protected static function selectMethode($request, $params)
    {
        ...
        } elseif ($request['methode'] == 'post' && !Res::isPost()) {
            $ok = 0;
        } elseif ($request['methode'] == 'get') {
            $ok = self::exec($params, $request);
            // break;  Problem with PHP7
        } elseif ($request['methode'] == 'resource') {
            $ok = self::exec($params, $request);
            // break;  Problem with PHP7
        } elseif ($request['methode'] == 'object') {
            $ok = self::exec($params, $request);
            //var_dump($request);
            // break;  Problem with PHP7
        }
                ...
                if (!is_null($value['subdomain'])) {
                    if (Collection::contains($value['subdomain'], self::getDomain())) {
                        if ($value['methode'] == 'post' && Res::isPost()) {
                            ...
                            break;
                        } elseif ($value['methode'] == 'post' && !Res::isPost()) {
                            $ok = 0;
                        } elseif ($value['methode'] == 'get') {
                            $ok = self::exec($params, $value);
                            break;
                        } elseif (is_array($value['methode'])) {
                            if ($value['methode']['type'] == 'resource') {
                                $ok = self::exec($params, $value);
                                break;
                            }
                        } elseif ($value['methode'] == 'object') {
                            $ok = self::exec($params, $value);
                                //var_dump($value);
                                break;
                        }
    ...
    protected static function runRoute($request, $params)
    {
        ...
        if (is_array($request['methode'])) {
            if ($request['methode']['type'] == 'resource' && $request['methode']['target'] == 'update') {
                ...
                $params[] = $id;
            } elseif ($request['methode']['type'] == 'resource' && $request['methode']['target'] == 'insert') {
                $params[] = new Request();
            }
        }
    ...
    protected static function showMaintenance()
    {
        ...
        } elseif (Config::get('maintenance.maintenanceEvent') == 'link') {
    ...
    protected static function addController($url, $controller, $methode, $params = false)
    {
        if ($methode == 'show' || $methode == 'edit' || $methode == 'delete') {
            $callback = function ($id) use ($controller, $methode) {
                return $controller::$methode($id);
            };
        } elseif ($methode == 'update') {
            $callback = function ($request, $id) use ($controller, $methode) {
                return $controller::$methode($request, $id);
            };
        } elseif ($methode == 'insert') {
            $callback = function ($request) use ($controller, $methode) {
                return $controller::$methode($request);
            };
        } else {
            $callback = function () use ($controller, $methode) {
    ...
    {
        $all = ['index', 'show', 'add', 'insert', 'edit', 'update', 'delete'];
        //
        if (isset($except)) {
            $i = 0;
    ...
    protected static function spliteStringCallback($string)
    {
        ...
        return ['controller' => $data[0], 'methode' => $data[1]];
    }
